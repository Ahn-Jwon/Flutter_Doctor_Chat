                      ...
                      return Padding(
                        ...
                        child: ElevatedButton(
                          ...
                          child: Row(
                            mainAxisAlignment: MainAxisAlignment.start,
                            children: [
                              Text(cardio_List[index]['cardio_insertdate']!),
                            ],
                          ),
                        ),
                      );
...
// ----------Function -------------
  ...
  _getList() async {
    ...
    // var url_diabetes = Uri.parse(
    //     "http://localhost:8080/Flutter/project/kkg_diabetesList.jsp?uid=$uid");
    // var response_diabetes = await http.get(url_diabetes);

    // var dataConvertedJSON_diabetes =
    //     json.decode(utf8.decode(response_diabetes.bodyBytes));
    //List result_diabetes = dataConvertedJSON_diabetes['diabetes'] ?? [];
    List result_diabetes = dataConvertedJSON['diabetes'] ?? [];
    diabetes_List.addAll(result_diabetes ?? []);

    // 치매 자가진단 내역 가져오기

    // var url_diamentia = Uri.parse(
    //     "http://localhost:8080/Flutter/project/kkg_diamentiaList.jsp?uid=$uid");
    // var response_diamentia = await http.get(url_diamentia);

    // var dataConvertedJSON_diamentia =
    //     json.decode(utf8.decode(response_diabetes.bodyBytes));
    // List result_diamentia = dataConvertedJSON_diamentia['diamentia'] ?? [];
